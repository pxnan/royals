...
const ManageDataset = ({ onDataChange }) => {
  ...
  const [editingId, setEditingId] = useState(null); // menyimpan id, bukan index
  ...
  const handleEdit = (item) => {
    setEditingId(item.id); // simpan id
    setEditForm({
  ...
  const handleUpdate = async () => {
    ...
    try {
      await updateData(editingId, editForm); // kirim id
      ...
      setError(err.message || "Gagal mengupdate数据");
    }
  };
